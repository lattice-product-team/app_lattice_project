...
- **Architecture: Independent Sheet Layer**: Instead of being a content overlay _inside_ the main island, the `ProfileSheet` will be a standalone component similar to `EventDetailSheet`. It will have its own `islandState` (SharedValue) and `GestureDetector`.
