...
- **Risk**: Nextra dev server caching during the docs rewrite.
  - _Mitigation_: We will instruct the developer to restart the Nextra server (`pnpm dev:docs`) to force a clean re-synchronization once all edits are completed.
