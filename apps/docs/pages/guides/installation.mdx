...
<div style={{ display: 'flex', gap: '16px', overflowX: 'auto', padding: '12px 0' }}>
  <img
    ...
    alt="Relational Spatial Maps (Dark Mode)"
  ...
  <img
    ...
    alt="Relational Spatial Maps (Light Mode)"
    style={{ height: '360px', borderRadius: '12px', boxShadow: '0 8px 30px rgba(0,0,0,0.12)' }}
  />
</div>
...
<Callout type="info">
  **Looking for the fast path?** If you are setting up your workspace for the first time using our
  standard Docker Compose stack, refer to the [Getting Started Guide](./getting-started.md). You do
  not need to install Postgres, PostGIS, or Redis natively on your host machine.
</Callout>
...
<Callout type="warning">
  If migrations or seeding fail, make sure the database is fully initialized and your `DATABASE_URL`
  contains the correct password and port. You can run `pnpm db:check` to diagnose connection issues.
</Callout>
