...
export const resolveAddress = async (req, res) => {
    try {
...
export const syncSocialData = async (req, res) => {
    try {
        const { type, id } = req.body;
        if (!type || !id) {
            return res.status(400).json({ error: 'Type (event/poi) and ID are required' });
        }
        const result = await socialService.syncAssetSocialData(type, parseInt(id, 10));
...
export const getEventSpatial = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const poisResults = await db
            ...
            geometry: sql `ST_AsGeoJSON(${pointsOfInterest.location})`,
        })
...
export const saveEventSpatial = async (req, res) => {
    try {
        const { id } = req.params;
        const eventId = parseInt(id, 10);
...
export const getGlobalStats = async (req, res) => {
    try {
        ...
        const [usersCount] = await db.select({ count: sql `count(*)::int` }).from(sql `users`);
        res.json({
...
export const getPoi = async (req, res) => {
    try {
        ...
        const metadata = poi.metadata ? JSON.parse(poi.metadata) : {};
        if (!metadata.social) {
            socialService
...
export const getPathNetwork = async (req, res) => {
    try {
      ...
      ${req.query.eventId ? sql `WHERE s.event_id = ${parseInt(req.query.eventId, 10)}` : sql ``}
    `);
...
export const getEvents = async (req, res) => {
    try {
        const results = await db
            ...
            center: sql `ST_AsGeoJSON(${events.location})`,
            boundary: sql `ST_AsGeoJSON(${events.boundary})`,
        })
            .from(events)
...
export const createEvent = async (req, res) => {
    try {
        const { name, description, type, startDate, endDate, locationName, address, boundary, primaryColor, isPermanent, center, } = req.body;
...
export const createPoi = async (req, res) => {
    try {
        const { eventId, name, description, type, geometry, locationName, address, capacity, isWheelchairAccessible, hasPriorityLane, } = req.body;
