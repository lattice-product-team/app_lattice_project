import { db, pointsOfInterest, sql, events, eq, and } from '@app/db';
...
export class PoiService {
  ...
  async getPois(category?: string, eventId?: string) {
    ...
    if (category && typeof category === 'string') {
      conditions.push(sql`${pointsOfInterest.type}::text = ${category}`);
    }

    if (eventId) {
      const eid = parseInt(eventId as string, 10);
      if (!isNaN(eid)) {
        conditions.push(eq(pointsOfInterest.eventId, eid));
      }
    }
