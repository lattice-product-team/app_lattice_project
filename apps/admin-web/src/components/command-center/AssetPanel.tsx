    ...
    <Card className="absolute right-8 top-[var(--admin-safe-area)] w-[380px] bg-white/40 backdrop-blur-xl border border-white/20 shadow-massive z-20 flex flex-col animate-in slide-in-from-right duration-500 rounded-[2.5rem] overflow-hidden max-h-[calc(100vh-var(--admin-safe-area)-4rem)]">
      ...
      <div className="flex-1 p-8 space-y-8 overflow-y-auto custom-scrollbar">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-end px-1">
            ...
            <span className="text-[10px] font-black text-obsidian">{asset.load || '45'}%</span>
          </div>
          <div className="h-1 bg-white/10 rounded-full overflow-hidden">
            <div className="h-full bg-obsidian transition-all duration-1000 ease-out" style={{ width: `${asset.load || 45}%` }} />
          </div>
        </div>
      ...
      <div className="p-8 py-6 flex gap-3 bg-white/10 border-t border-white/10">
        <button 
          ...
          className={cn(
            "flex-1 h-11 rounded-xl text-[10px] font-bold uppercase tracking-[0.15em] border border-white/20 transition-all active:scale-95 shadow-sm",
            isRadarActive 
              ? "bg-signal-blue text-white border-signal-blue shadow-lg scale-105" 
              : "bg-powder/50 text-obsidian hover:bg-powder"
          )}
        >
        ...
        <Button 
          ...
          className="flex-1 h-11 rounded-xl bg-obsidian text-white text-[10px] font-bold uppercase tracking-[0.15em] hover:bg-obsidian/80 active:scale-95 transition-all shadow-massive"
        >
          Manage
        </Button>
      </div>
