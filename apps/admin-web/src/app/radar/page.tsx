    ...
    fetch(`${API_BASE}/events/${selectedEventId}/spatial`)
      ...
      .then((data) => {
        ...
        if (boundaryFeature) {
          ...
          setViewState((prev) => {
            if (prev.longitude === lng && prev.latitude === lat) return prev;
  ...
  return (
    <div className="relative w-full h-screen bg-eggshell overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <Select.Popover>
                  <ListBox
                    items={events}
                    className="bg-white border border-chalk rounded-xl p-1 min-w-40 shadow-subtle"
                  >
                    {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                    {(v: any) => (
                      <ListBox.Item
                        id={v.id.toString()}
                        textValue={v.name}
                        className="flex items-center px-3 py-1.5 rounded-lg text-admin-xs font-medium text-gravel hover:bg-powder cursor-pointer outline-none focus:bg-powder"
                      >
                        {v.name}
                      </ListBox.Item>
                    )}
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-gravel">
                  Live Stream
                </span>
              </div>
            </div>
      ...
      <Card className="absolute right-6 bottom-6 w-52 bg-white/80 backdrop-blur-md p-4 border-chalk shadow-subtle z-10">
        <h4 className="text-[9px] font-black uppercase tracking-widest text-gravel mb-3 border-b border-chalk pb-2 text-center">
          Crowd Density
        </h4>
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-black text-slate uppercase tracking-tighter">
              Sparse
            </span>
