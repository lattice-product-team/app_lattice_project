...
export default function POIsPage() {
  ...
  const [isWheelchairAccessible, _setIsWheelchairAccessible] = useState(true);
    ...
    <div className="space-y-12 px-8 py-12 pb-24">
      ...
      <Modal isOpen={isRegisterModalOpen} onOpenChange={setIsRegisterModalOpen}>
        <ModalBackdrop>
          <ModalContainer
            size="lg"
            className="bg-white border border-chalk rounded-2xl p-0 shadow-subtle overflow-hidden"
          >
            <ModalDialog className="outline-none">
              <div className="flex flex-col h-[700px] max-h-[90vh]">
                ...
                <div className="w-full h-[250px] bg-powder/30 relative shrink-0 border-b border-chalk">
                  ...
                  <div className="absolute top-4 left-4 z-10">
                    <div className="bg-white/90 backdrop-blur-sm px-3 py-2 rounded-lg border border-chalk shadow-sm max-w-[200px]">
                      <p className="text-[10px] font-black uppercase tracking-widest text-obsidian mb-0.5">
                        Location Picker
                      </p>
                ...
                <div className="w-full flex-1 p-6 overflow-y-auto space-y-6 bg-white">
                  <ModalHeader className="waldenburg-display text-admin-xl text-obsidian p-0">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <Select.Popover>
                          <ListBox
                            items={events}
                            className="bg-white border border-chalk rounded-xl p-1 min-w-[300px] shadow-subtle max-h-60 overflow-y-auto"
                          >
                            {(e: { id: number | string; name: string }) => (
                              <ListBox.Item
                                id={e.id.toString()}
                                textValue={e.name}
                                className="flex items-center px-3 py-2 rounded-lg text-admin-xs font-medium text-gravel hover:bg-powder cursor-pointer outline-none focus:bg-powder"
                              >
                                {e.name}
                              </ListBox.Item>
                            )}
          ...
          <table className="w-full text-left border-collapse min-w-[1300px]">
            <thead>
              <tr className="border-b border-chalk bg-powder/50">
                <th className="py-4 px-6 text-gravel uppercase text-[10px] tracking-widest font-black">
                  ID
                </th>
                <th className="py-4 px-6 text-gravel uppercase text-[10px] tracking-widest font-black">
                  Asset Details
                ...
                <th className="py-4 px-6 text-gravel uppercase text-[10px] tracking-widest font-black text-center">
                  Accessibility
                </th>
                <th className="py-4 px-6 text-gravel uppercase text-[10px] tracking-widest font-black">
                  Status
                </th>
                <th className="py-4 px-6 text-gravel uppercase text-[10px] tracking-widest font-black text-right">
                  Actions
                </th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                pois.map((poi: any) => {
                  ...
                  return (
                    <tr
                      key={poi.id}
                      className="border-b border-chalk hover:bg-powder/30 transition-colors group"
                    >
                      <td className="py-6 px-6">
                        <span className="font-mono text-admin-xs text-slate">
                          POI-{poi.id.toString().padStart(3, '0')}
                        </span>
                      </td>
                      <td className="py-6 px-6">
                        <div className="flex flex-col">
                          <span className="font-bold text-obsidian text-admin-base">
                            {poi.name}
                          </span>
                          <span className="text-[11px] text-gravel uppercase tracking-wider font-medium">
                        ...
                        {social ? (
                          <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-[10px] text-gravel font-medium">
                                ({social.reviews_count})
                              </span>
                            </div>
                      ...
                      <td className="py-6 px-6">
                        <div className="flex flex-col max-w-[250px]">
                          <span className="text-admin-base font-bold text-obsidian truncate">
                            {poi.locationName || 'Location Pending'}
                          </span>
                          <span
                            className="text-[11px] text-gravel leading-tight truncate"
                            title={poi.address}
                          >
                            {poi.address || 'Coordinates resolved'}
                          </span>
                        </div>
                      </td>
                      <td className="py-6 px-6">
                        <div className="flex flex-col gap-2 w-32">
                          <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-tighter text-obsidian">
                            ...
                            <span>
                              {poi.currentOccupancy || 0} / {poi.capacity || 0}
                            </span>
                          </div>
