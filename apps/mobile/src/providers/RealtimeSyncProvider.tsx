...
export const RealtimeSyncProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const throttleInvalidate = useCallback(
    (keys: string[]) => {
      const now = Date.now();
  ...
  useEffect(() => {
    ...
    // 1. Sync POIs
    const unsubscribePois = subscribe(
      'sync:pois',
      (data: { action: string; id: number; eventId?: number }) => {
  ...
  return (
    <RealtimeSyncContext.Provider value={{ isConnected }}>{children}</RealtimeSyncContext.Provider>
  );
};
