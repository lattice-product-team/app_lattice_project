...
async function seed() {
  ...
  await db
    ...
    .onConflictDoNothing();
  const [neonNights] = await db
    .select()
    .from(events)
    .where(eq(events.name, 'Neon Nights Festival'));
  ...
  await db
    ...
    .onConflictDoNothing();
  const [quantumConf] = await db
    .select()
    .from(events)
    .where(eq(events.name, 'Quantum Tech Summit'));
      ...
      pois: [
        {
          ...
          type: 'meetup_point',
          coords: [2.2592, 41.5695] as [number, number],
        },
      ...
      pois: [
        {
          name: 'Solar Stage',
          type: 'meetup_point',
          coords: [2.2235, 41.4135] as [number, number],
        },
        {
          name: 'Ocean Stage',
          type: 'meetup_point',
          coords: [2.2255, 41.4115] as [number, number],
        },
        {
          name: 'Chill Out Zone',
          type: 'restaurant',
          coords: [2.2215, 41.4125] as [number, number],
        },
      ],
      nodeOffset: 200,
    },
      ...
      pois: [
        {
          ...
          type: 'meetup_point',
          coords: [2.1315, 41.3545] as [number, number],
        },
  ...
  for (const config of eventConfigs) {
    ...
    const segments = [
      { s: nodeIds[0], t: nodeIds[1], dist: 150, stairs: false, surface: 'asphalt' },
      {
        ...
        t: nodeIds[2],
        dist: 180,
        stairs: config.event.type === 'sports',
        surface: 'gravel',
      },
