...
async function generateQRs() {
  ...
  for (const event of allEvents) {
    ...
    const eventPois = await db.select().from(pointsOfInterest).where(eq(pointsOfInterest.eventId, event.id));

    // Intelligence: Find "Grandstand" (Grada) or "VIP" or "Stage"
    const grandstand = eventPois.find(p => p.name.toLowerCase().includes('grandstand') || p.name.toLowerCase().includes('grada') || p.name.toLowerCase().includes('tribuna'));
    const vipZone = eventPois.find(p => p.name.toLowerCase().includes('vip') || p.name.toLowerCase().includes('meetup'));
    
    const zones = [];
    
    if (grandstand) {
      zones.push({ name: grandstand.name, gate: 'Porta A', row: '12', startSeat: 4, count: 2, location: grandstand.location });
    }
    
    if (vipZone) {
      zones.push({ name: vipZone.name, gate: 'VIP Entrance', row: 'A', startSeat: 1, count: 2, location: vipZone.location });
    }

    // Always add a general admission if no special zones found or as a base
    zones.push({ name: 'General Admission', gate: 'Main Entrance', row: null, startSeat: null, count: 2, location: event.location });
